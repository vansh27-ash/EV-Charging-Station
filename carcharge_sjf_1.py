...
`    if x[1]>=60:
    ...
    for i in range(1,len(l)):
        ...
        for j in range (1):
            try :
                if l[i][1][0] >= l3[j][3][0]:
                    if l[i][1][0]==l3[j][3][0]: #if reach of upcoming car is equal to leaving time of gone car comparing only hrs
                        if l[i][1][1]>=l3[j][3][1]:
                    ...
                    elif l[i][1][0]>l3[j][3][0]: #if hrs of upcoming car is greater than hrs of gone car
                        x=[l[i][1][0]+l[i][2][0],l[i][1][1]+l[i][2][1]]
    ...
    writer_object.writerow(["sjf",str(profit),str(len(l1))])
    f.close()
    f1.close()
